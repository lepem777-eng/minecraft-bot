...
if [ ! -d "$CLIENT_HOME" ] || [ ! -f "$CLIENT_HOME/version.json" ] || [ ! -f "$CLIENT_HOME/${MINECRAFT_VERSION}.jar" ] || [ ! -d "$CLIENT_HOME/libraries" ] || [ ! -d "$CLIENT_HOME/assets/objects" ]; then echo 'ERROR: MINECRAFT_CLIENT_RUNTIME_MISSING' >&2; exit 2; fi

...
const createFullscreenViewer = require('./server/fullscreen');
...
const fullscreenServer = createFullscreenViewer(config, viewer);
fullscreenServer.listen(config.viewer.publicPort, config.viewer.host, () => logger.info(`Fullscreen POV listening on http://${config.viewer.host}:${config.viewer.publicPort}`));
async function shutdown(signal) { logger.info(`Received ${signal}; shutting down.`); botManager.shutdown(); fullscreenServer.close(); server.close(() => process.exit(0)); setTimeout(() => process.exit(0), 3000).unref(); }

...
The dashboard/API binds to `PORT` (or `WEB_PORT`). The fullscreen POV relay separately binds to `VIEWER_PORT` (default `3001`) on `0.0.0.0`; configure a separate Railway TCP/public networking service or `VIEWER_URL` for that port. Railway does not expose a second port merely because `VIEWER_PORT` is set.

The Docker build obtains the complete official Mojang 1.20.1 runtime (version metadata, client JAR, libraries, and assets) through `npm run install:minecraft`; this requires normal outbound Mojang download access during the image build. For a Forge profile, deploy the compatible client mod JARs (do not download unknown mods) to `minecraft/mods`, install the Forge profile with `npm run install:forge`, and install a compatible headless bridge under `minecraft/client/start.sh`. The dashboard always launches `scripts/start-forge-client.sh` by default. Run `npm run diagnose` before deployment. The dashboard never returns authentication credentials or runner environment variables.
